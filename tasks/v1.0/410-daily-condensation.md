...
- **Drift from plan:** — (e.g. whether `json_extract` was available or `list_daily_summaries` fell back to a `LIKE` filter; whether `InputWindow` was returned as a struct or a rendered `String`; exact day-boundary math for the summary `created_at`.)
- **Open questions for next task:** — (Task **414** drives `condense_day` from a timer/boot tick and feeds `assemble_input_window`'s `InputWindow` to the spawned Maestro agent's stdin — built on the FROZEN `condense.rs` surface + the maestro chat id from 403; Task **412**'s real Haiku/Sonnet provider replaces `DeterministicOneShot` as the live summarizer behind `OneShotLlm`.)
- **Deliberate debt:** — (the LIVE summarizer is `DeterministicOneShot` truncate/collapse so the Tier-1 bench is reproducible; real-LLM summary *quality* is 412's, not stubbed with a macro — `condense_day` returns `Ok(None)` when there is nothing to condense, never `todo!()`. The flat 7-day `WEEKLY_SUMMARY_WINDOW` has no coarser rollup tier — older summaries persist for the UI but leave the agent window; a future task may add monthly rollups.)
- **Smoke-gate state:** — (expected **unchanged**; 410 touches no `scripts/smoke.d/*` or `scripts/smoke.manifest`; the live daily-condensation cadence is turned on by 414.)
