...
- `crates/core/src/handlers/vcs.rs` (modified — D10 fix: resolved `enterprise_data_privacy` replaces the hardcoded `false`; adds the `EnterprisePrivacyResolver` trait + `CorePrivacyResolver` + `VcsHandler::with_privacy_resolver`)
- `crates/core/src/boot.rs` (modified — documented drift: injects `MaestroConeSuggester` via the FROZEN `with_cone_suggester` injector + builds the `CorePrivacyResolver` and threads it; `repo_manager/actor.rs`'s injector was reused unchanged, so `actor.rs` is NOT modified)
- `crates/core/src/api_server.rs` + `crates/core/src/connect_bridge.rs` (modified — documented drift: additive `vcs_privacy_resolver` field/builder alongside 414's maestro wiring, consumed at the `VcsHandler` construction site at both serve fronts)
...
- **Drift from plan / D10 fix mechanics + boot drift:** The D10 fix replaces the hardcoded `enterprise_data_privacy: false` in `handlers/vcs.rs::fetch_issue_by_url` with a value resolved BEFORE any keychain/outbound call. The resolution is done by a new `EnterprisePrivacyResolver` trait + a `CorePrivacyResolver` impl (both in `handlers/vcs.rs`): a `workspace_id` scope routes through the FROZEN `settings::build_resolver_for_workspace` factory and reads `enterprise_data_privacy().value`; an empty scope (or a resolution error) falls back to the Core-wide `ManagedPolicy::enterprise_data_privacy()` floor (default `false`) — a privacy-forcing managed Core still blocks Linear/Jira even when no workspace is supplied (GitHub is never gated; that arm is the vcs router's job). The resolver is attached to `VcsHandler` via the additive `with_privacy_resolver`. **Boot drift (threaded ALONGSIDE 414's maestro wiring, not into it):** (a) `boot.rs` builds `CorePrivacyResolver` as its OWN self-contained `let vcs_privacy_resolver` (from `load_managed_policy(config_dir)` + `OptOutConfig::load(~/.concerto)` + `Arc::clone(&persistence)`), wired into the `ApiServerActor` factory via a new `.with_vcs_privacy_resolver(..)` builder AND into the Iroh `CoreServiceSet` field; (b) `boot.rs` also injects the Maestro `ConeSuggester` as a separate `#[cfg(unix)] let repo_handle = repo_handle.with_cone_suggester(MaestroConeSuggester::new(repo_handle.clone(), default_oneshot()))` right after `with_prewarm_signals` — `RepoManager::actor.rs` was NOT modified (its FROZEN `with_cone_suggester` injector was reused as-is). `api_server.rs` gained an additive `vcs_privacy_resolver` field on `CoreServiceSet`/`ApiServerActor` (+ `run_uds` param) wired in `add_core_services` ALONGSIDE 414's existing `maestro` field; `connect_bridge.rs` gained the same additive `BridgeServices.vcs_privacy_resolver` field consumed in `build_and_serve` next to 414's `maestro` field. **414's maestro `boot.rs`/`api_server.rs`/`connect_bridge.rs` wiring (the `maestro_handle` block, the `maestro` field, `with_maestro_events`, the `MaestroServer` registration) was preserved verbatim — every 411 addition is a distinct field/let.** `maestro/mod.rs` gained `pub mod cone_suggester;` + `pub use ...MaestroConeSuggester` in its OWN distinct Task-411 region below 414's `pub mod events;`.
- **Open questions for next task:** **Task 415** (Desktop create-from-description UX) consumes the FROZEN `Repositories.SuggestCones` wire surface (`SuggestConesRequest{repository_id=1, issue_text=2}` / `SuggestConesResponse{cone_paths=1}`) for its cone-picker, and renders the create-flow confirmation chip slate the `create_from_description` planner proposes (the three §3.8 step-4 chips carry the FROZEN action tokens `create_workspace_from_description:with_workarea` / `:workspace_only` / `:edit_repos`; on the "with_workarea"/"workspace_only" confirm, the Desktop drives `resolve_create_plan`). **Task 413** owns the rest of D10's privacy enforcement (the per-workarea summary/digest gate + `concerto_chat_full_chat_access`) and can reuse the same per-workspace `enterprise_data_privacy()` resolver this task now binds to `fetch_issue_url`.
- **Deliberate debt:** The `SuggestCones` RPC returns `Status::unimplemented` only if a Core is built WITHOUT the injector (e.g. a non-unix Core where `MaestroConeSuggester` is `#[cfg(unix)]`-absent, or a unit test that omits it) — a runtime `tonic::Status` via the FROZEN `cone_suggest_error_to_status`, NOT the `unimplemented!()` macro and never an empty success. The live cone-suggestion quality is the deterministic ranking (issue-keyword ∩ real top-level dirs, fallback to the first N real dirs) over 312's `DeterministicOneShot` until **Task 412** swaps the real provider in behind the same `Arc<dyn OneShotLlm>` (zero change here). The `create_from_description.workspace_id_hint` arg is reserved (carried but unused) for a future "add to existing workspace" flow.
- **Smoke-gate state:** Unchanged. 411 adds no smoke capability; the create-flow planner + `SuggestCones` + the D10 resolver are all CI-proven via the in-process harness (lib tests in `maestro/tools/write.rs` + the `tests/suggest_cones_411.rs` integration suite over a tempdir SQLite DB + a `file://` bare-repo fixture + a stub `ConeSuggester`). The 313 wiremock + the real `fetch_issue_url` round-trip stay the Phase-4 Tier-3 line "create a workspace from a real issue link"; the real `fetch_issue_url` enterprise-privacy gate is covered at the `crates/vcs` layer (`clients_linear_jira`). NOTE: highest migration on base is now `0016` (403's `0015_maestro_state` + 410's `0016_chat_messages_metadata` landed); 411 adds none, as planned.
