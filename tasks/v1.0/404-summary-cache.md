...
- **Drift from plan:** `ci_state` ended up **`Option<String>`** (the frozen public-interface block in this file is authoritative and uses `Option<String>`, not the `design/08 §3.3` typed `CiState` enum). Same for `status`/`pr_state`/`blocked_on` — all opaque `String`/`Option<String>` so consumers read one shape and never re-derive a typed enum. The `checks.<wa>.<repo>` opaque frame shape is **not yet stable**, so `parse_ci_state(&[u8]) -> Option<String>` is a TOTAL, conservative parser: it recognizes only a plain UTF-8 token equal to a canonical CI state (`pending|running|success|failure|error|cancelled|neutral`) and returns `None` for anything else (binary/length-framed payloads included). `ci_state = None` therefore means "unknown / not-yet-parseable", documented in the fn doc-comment; no `todo!()`/`unimplemented!()`. The **clock seam is a trait** (`trait Clock { fn now_ms(&self) -> i64 }`) with `SystemClock` (prod) + `ManualClock` (atomic-backed, advanceable behind `&`) — chosen over a bare `Fn` so the cache can own a `Box<dyn Clock>` and tests swap it. `rust-api.md` **did NOT change** (the 305 free-fn/`src/maestro/*`-omission caveat held — regen reads `src/api.rs` struct/enum defs, not `maestro::summary`; `commits_ahead` is a free fn). Regen was still run; it produced no diff. **`AgentKind` in `SessionSummary` is the existing `agent_supervisor::actor::AgentKind`** (Echo/Claude/Codex/Gemini today); 402 adds the `Maestro` variant additively — no impact on this shape.
- **Open questions for next task:** The field set is **sufficient for 405/409/413/415**. `SummaryCache::get(&WorkareaId) -> Option<WorkareaSummary>` (cheap clone) is the 405 read path; `list() -> Vec<WorkareaSummary>` is the 409 `list_active_summaries` snapshot, paired with `force_refresh_if_stale(wa, 60_000, rebuild)` for the on-`GetDigest` path (note: it takes a `rebuild` closure and only runs on the stale branch, and returns `None` for an **untracked** workarea — the caller must `upsert` a seed entry first; `is_stale` reports a missing entry as stale by design). For **413 (privacy)**: the shape cleanly separates blank-able LLM text (`last_turn_summary`, `last_3_turn_summaries`, `sessions[].last_turn_summary`) from the always-visible hard facts (`status`, `branch_name`, `repos[]` incl. `commits_ahead`/`files_changed`/`lines_*`/`pr_state`/`ci_state`, `composer_name`, `workspace_name`) — so `exclude_from_maestro → name-only` blanks the three summary fields while leaving `repos`/status intact, and `concerto_chat_full_chat_access` extends `last_3_turn_summaries` to raw turns. 413 will add the blanking pass over this same `summary.rs` (shared file, per §8.1). The `generation: u64` bump is in-process only; **414** turns it into the on-wire "summary updated" hint.
- **Deliberate debt:** (1) `ci_state` defaults to `None` until the `checks.<wa>.<repo>` opaque-frame schema is stabilized downstream — the `parse_ci_state` entry point is FROZEN so that task fills the body without touching call sites. (2) The summarizer uses `DeterministicOneShot` only (the LIVE P4 path, which for `DigestSummary` echoes the whitespace-collapsed context); real Haiku/Sonnet summary quality is deferred to **Task 412** + the **Phase-4 Tier-3 gate**. (3) The cache's wiring to a *live* `subscribe_events` loop + `workarea.events` feed is exposed as the trigger methods (`on_turn_complete`/`on_status_change`/`force_refresh_if_stale`/`is_idle`) consumed by an owner actor; this task proves them via in-process injection (the Tier-2 double), the actual long-lived subscriber task lands where the Maestro spine is wired (402/414). **No `todo!()`/`unimplemented!()` macro is used anywhere** (the only textual match is a doc-comment describing the discipline).
- **Smoke-gate state:** **Unchanged** — 404 is CI-provable via in-process event injection + a synthetic clock + the deterministic summarizer; `commits_ahead` is a plain `git rev-list --count` shell-out via the existing `gix_wrap::cmd::run` helper (no new crate, no new `gix` feature — `cargo deny` green). No `scripts/smoke.d/*` or manifest change. Migration high-water mark on `main` at impl time is **`0015`** (Task 403's `maestro_state`); 404 adds **no migration** (in-memory `HashMap`).
  - **Tier-2 double — what it does NOT cover:** the double (synthetic `AgentEvent::TurnComplete`/`workarea.events` injection + `ManualClock` + `DeterministicOneShot`) proves the cache shape, every refresh trigger, the `commits_ahead`/diff-count/`pr_state`/`ci_state` hard-fact derivation, and the staleness/idle logic fully in CI. It does **NOT** cover **real-LLM summary quality** (whether a Haiku/Sonnet rolling summary is actually useful) — that is Task 412's provider, judged at the **Phase-4 Tier-3 checklist line**: "leave for >30 min across active workareas, return, judge digest quality + measure latency."
