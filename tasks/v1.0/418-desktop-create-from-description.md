...
## Definition of Done
- [ ] `Repositories.SuggestCones` binding + `RpcMethod` arm
- [ ] Discoverable "create from description / issue link" entry point; flow shows editable detected repos + suggested cones (reused `ConePicker`)
- [ ] Confirmation slate (§3.8 options) drives create ONLY on explicit confirm (no silent create, R-2)
- [ ] Tier-2 tests pass (`pnpm -C apps/desktop typecheck|lint|test|build`); smoke unchanged
- [ ] No TODO/FIXME/unimplemented in new code; no `src-tauri`/proto/Rust; only `apps/desktop/**`
