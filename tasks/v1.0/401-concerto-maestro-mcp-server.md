...
- **Drift from plan** — <e.g. the exact `rmcp` version + whether its server API matched the transcribed `serve_maestro_mcp`/`ToolDescriptor` shape; the precise in-process stdio framing chosen (pipe pair vs. duplex stream) and how 400's pin maps to the `rmcp` transport type; whether `regen-interfaces.sh` captured the `maestro` registry types into `rust-api.md`; whether 0014 was still the migration high-water mark (no migration in this task either way).>
- **Open questions for next task** — <Task **402** consumes this server: it adds `AgentKind::Maestro` + `ToolClass::ReadOnly` and dials `serve_maestro_mcp`'s stdio endpoint via `--mcp-config` + `--strict-mcp-config` (PHASE4_PLANNING §4.8); confirm the endpoint constructor's signature is the one 402 expects. Tasks **405/406/407** build behind the FROZEN 16-tool schemas — each adds a `pub mod {read,write,side};` line in `tools/mod.rs` (lead-owned seam) + replaces its tools' dispatch arms; confirm the `ToolDescriptor`/`dispatch` shape lets them do so without re-shaping any schema. Task **404** adds `summary.rs` in the soft-seam zone and aligns `get_workarea_summary`'s output_schema to `WorkareaSummary` (§4.4).>
- **Deliberate debt** — <the 16 typed-unimplemented dispatch arms (the explicit FROZEN 401 behavior, wired live in 405/406/407) — each returns a typed `rmcp`/`McpError` not-implemented, NOT the macro and NOT empty-success; list any tool whose output_schema was authored as a minimal placeholder pending its consumer (e.g. `get_workarea_summary` ← 404).>
- **Smoke-gate state** — <**unchanged** (not re-run / re-run): 401 adds no smoke capability — the server is registered-but-unwired (no Maestro spawn until 402), so `scripts/smoke.sh` has nothing new to exercise; confirm `cargo check --workspace` green and the Windows lane compiles via the `cfg(unix)` gate.>
