...
- **Drift from plan:**
  - **Signature — `persist: &Persistence` not `pool: &SqlitePool`.** The §4.4-sketched frozen signature ended in `pool: &SqlitePool`, but `generate_digest` must *write* (the digest `chat_messages` row + `maestro_state.last_digest_at`), and writes go through `Persistence::writer()` (`tokio::sync::Mutex<SqliteConnection>`), not a read-only `SqlitePool`. So the last arg is `persist: &Persistence` — the same handle 410's `condense.rs` consumes (verified sibling pattern). Reads (the maestro-chat lookup) use `persist.readers()` underneath. The summary cache is taken as `&SummaryCache` (404's frozen handle, by reference — not an `Arc`).
  - **`<5 s p50` landed as a timed `#[test]`, not a Criterion bench.** The task file explicitly allowed "OR an in-module timed `#[test]` if a bench harness is not warranted — decide minimally." Chosen because a Criterion bench would need a tokio runtime + a live `Persistence` DB in the bench harness (the digest is async + writes SQLite), which is heavier than the budget-guard warrants. The timed test (`digest_latency_p50_under_5s_on_six_workarea_fixture`) runs the LLM-bound hot path (delta+group+prompt+`DeterministicOneShot`) 50× over the 6-workarea fixture and asserts p50 < 5 s (deterministic path is sub-ms). **No `crates/core/benches/digest_latency.rs` was added** — so `criterion` did NOT need adding (it is already a `crates/core` dev-dep at Cargo.toml:243 regardless), and `cargo deny` stayed green with **zero new crates** (no Stop-and-ask triggered).
  - **`chats(kind='maestro')` singleton id is looked up, not passed in.** `generate_digest` takes `workspace_id`, not a chat id; it resolves the singleton via a `SELECT id FROM chats WHERE kind='maestro' LIMIT 1` read (403's `ensure_maestro_chat` has no getter). When the singleton is absent (414 bootstraps it at boot), `generate_digest` returns a typed `Error::NotFound` rather than silently dropping the chips — the digest itself is still assembled (a caller could render it).
  - **`compute_delta` reads the current snapshot conservatively.** 404's `SummaryCache` carries only the *current* `WorkareaSummary` (no historical generations), so "what advanced since `last_seen_at`" is computed from the current hard facts (`commits_ahead`/`files_changed` totals, `pr_state`/`ci_state` presence) + `last_activity_at > last_seen_at`. There is no prior-snapshot diff to subtract against; documented inline.
  - **`docs/interfaces/` delta: none** (confirmed `git diff --exit-code docs/interfaces/` clean — `crates/core/src/maestro/*` is not scanned by `regen-interfaces.sh`, matching 305's finding). Highest migration on base is **0016** (0015=403, 0016=410's `chat_messages.metadata`); 409 adds none. Note: 410's `metadata` column **is already on this base**, but the digest row deliberately writes `metadata: None` (a digest is a plain `assistant` message, not a `daily_summary`).
  - **404's frozen `WorkareaSummary` field names matched verbatim** (`status: String`, `last_activity_at: i64`, `blocked_on: Option<String>`, `repos: Vec<RepoSummary>` with `commits_ahead`/`files_changed`/`pr_state`/`ci_state`) — no re-shaping. `classify_group` takes the computed `&WorkareaDelta` only (the summary's status/`blocked_on` are already folded in by `compute_delta`).
- **Open questions for next task:** **Task 414** consumes `generate_digest()` (the FROZEN `Digest`/`WorkareaDelta` surface) to serve `Maestro.GetDigest` + publish `maestro.digest_generated`. The Core `Digest` carries structured `finished`/`blocked`/`working` groups + `next_step`, but 401.5's `Digest` proto is flat (`text`/`chips`/`generated_at_ms`/`stale`) — **414 folds the groups + `next_step` into the wire `text`** (the `one_line` on each `DigestEntry` is render-ready) and maps `chips` 1:1 onto `MaestroChip` (same field set) and `degraded → stale`. Confirm whether 414 wants `generate_digest` to emit the event itself or leave that to the handler (current contract: 409 only generates + persists; **the handler emits**). **Task 412** swaps the injected `Arc<dyn OneShotLlm>` for the real provider — the injection point (`default_oneshot()` / the `llm:` arg) is the only change needed; the deterministic double does NOT cover **real-LLM digest quality** (whether a real Sonnet call writes a *good* 3–5-sentence summary) or **real-LLM latency** (whether the network call meets <5 s p50) — both defer to the **Phase-4 Tier-3 gate** ("leave >30 min across active workareas, return, judge quality + measure latency"). **Task 407**'s `ChipSlate` is NOT yet merged; chips are persisted per this task file's own instruction (D11 — into the digest's `content_json` envelope), so no 407 dependency was needed and no block occurred; if 407 later wants the digest's chips in its slate, the `DIGEST_CHIP_RULE_ID`-tagged chips in the persisted envelope are the seam.
- **Deliberate debt:** Chips ride inside `content_json` (a `{kind:"digest", text, groups, next_step, chips[], degraded}` JSON envelope), **not** a `metadata` column — even though 410's `metadata` is on this base, a digest is a plain `assistant` message (no `daily_summary` tag), so no migration of existing digest rows is ever needed and 410 needs no rework. The degraded-LLM path returns a typed `Digest{degraded:true, text:DEGRADED_DIGEST_TEXT}` with the deterministic groups+chips still present and **still persisted** (so a returning user keeps a clickable surface) — the explicit R-7/§8 contract, never a `todo!()`/`unwrap()`. Stale facts (a summary older than the 60 s window) also set `degraded` so 412's stale-badge fires; the read-only `&SummaryCache` is not mutated here (404/414 own the force-refresh write).
- **Smoke-gate state:** **Unchanged** (409 touches no `scripts/smoke.d/*` or `scripts/smoke.manifest`; the digest is CI-provable in-process via `DeterministicOneShot` + the 6-workarea fixture; the live `maestro-digest` smoke capability + real-LLM latency are 414's wire and the Phase-4 Tier-3 gate). `cargo deny` stayed **green** — **no new runtime crate**; the `<5 s` proof is a timed `#[test]`, so not even the dev-only `criterion` was added.
```
