...
- **Drift from plan** — _(fill at impl time)_ e.g. whether `<MaestroChat>` mounted in `App.tsx` vs as a top region of `AppLayout`; whether the `Maestro.*` arms were added to the `client.ts` `RpcMethod` union now (TS-type-only, 414 adds the matching Rust dispatch arm) or kept local to `maestro.ts` until 414; the exact `Digest`/`MaestroMessageRequest`/`VisibilityRequest` field names reconciled against 401.5's committed `maestro.proto` (note any divergence from `design/08 §5.3`'s sketch); any `@testing-library/react`/`jsdom` devDeps added vs already present from 218/322/323.
- **Open questions for next task** — _(fill at impl time)_ **Task 414** is the consumer that lights up this UI: it fills 401.5's `Status::unimplemented` `MaestroServer`, adds the `Maestro.*` arm to the Rust shell `commands.rs` dispatch + publishes `maestro.events` on `Event.checks_opaque=17` — confirm the live frame shape matches this renderer's defensive decoder (the FROZEN surface it builds on is 401.5's §4.2). Note whether the live digest/chip field names matched the mirror types exactly (they must, per §4.2) or required a one-line mirror-type rename.
- **Deliberate debt** — _(fill at impl time)_ the live-data path is mocked (`invoke` double) by design until 414; any empty-state / "Maestro not yet wired" placeholder render is a deliberate UX seam (NOT a `todo!()`), documented here. Settings → Concerto Chat backend/budget config UI deliberately out (provider seam is 402/412).
- **Smoke-gate state** — _(fill at impl time)_ unchanged; `apps/desktop/src`-only task, no smoke capability touched; `scripts/smoke.sh` is the CI/operator gate for an `unchanged` task and was not re-run in-worktree. Confirm `pnpm -C apps/desktop typecheck|lint|test|build` all green (the §7 override).
