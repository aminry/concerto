...
- **Drift from plan:** —
- **Open questions for next task:** — (name the consuming tasks: **409** matches `SlashDirective::Digest` to generate the digest; **414** runs every `SendToMaestro` input through `pre_parse` → `resolve_targets` → `dispatch`, owns the cross-workspace `AmbiguousComposer` branch + the synthesized assistant message + the `maestro.routing_executed` event; **406**'s `route_prompt_to_session`/`fanout_to_sessions` write-tools may reuse `resolve_targets`/`dispatch`. All build on the FROZEN `ParseOutcome`/`RoutingTarget`/`SlashDirective`/`RoutingError` surface.)
- **Deliberate debt:** — (`@idle`/`@blocked` classify from raw `Workarea`/`Session` status columns, not the 404 `WorkareaSummary`/`BlockedReason` cache, to keep 408 dependent only on 402; note if 404/413 should later refine the classifier.)
- **Smoke-gate state:** — (unchanged; routing adds no smoke capability — the maestro-digest smoke check is owned by 409/414.)
