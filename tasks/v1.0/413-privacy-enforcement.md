...
- **Drift from plan:** None material. Specifics: (a) **Fields blanked vs. kept** — `blank_excluded` sets `last_turn_summary = "[private workarea, name only]"` (exact `PRIVATE_WORKAREA_BLANK`), empties `last_3_turn_summaries`, and **empties `sessions`** (the stronger guarantee — a `SessionSummary` carries only `model`/`status`/`last_turn_summary`, no hard fact the UI needs for a private workarea). Everything else stays: `workarea_id`, `workspace_id`, `workspace_name`, `composer_name`, `branch_name`, `status`, `last_activity_at`, the whole `repos` `Vec` (commits_ahead/files_changed/lines_*/pr_state/ci_state), `blocked_on`, `generated_at`, `generation` — each asserted positively in `excluded_leaks_only_hard_facts`. (b) **`is_external_model`** landed as a `bool` on the policy methods (`maestro_disabled_by_policy`/`llm_gate`) **plus** a typed `MaestroModelLocality { External, OnPrem }` input enum with `.is_external()` for 412 to construct from — no constant is hardwired; 413 takes the input, 412 supplies it. (c) **`concerto_chat_full_chat_access` reader lives ONLY in the persist accessor** (`workspaces::get_settings_json_bool`) — the `WorkspaceSettingsResolver` is built from pre-parsed managed/checked-in/local-db settings injected at construction and does **not** load the workspace `settings_json` blob, so adding a resolver getter would have meant changing its construction; per the task's "otherwise read the bool via the persist accessor at the call site and keep the resolver untouched", `resolver.rs` is **untouched**. (d) **`summary.rs` had no serve-side privacy seam** — added `get_for_maestro`/`list_for_maestro` (read-time `blank_excluded`, raw `get`/`list` retained for non-Maestro callers), `summarize_turn_gated` (the external-call gate), `refresh_summary_source` (the `SummarySource` toggle), and `MAESTRO_DISABLED_BY_POLICY_SUMMARY = "[maestro disabled by policy]"` marker. Blanking is **serve-time** (a freshly-flipped `exclude_from_maestro` is honored on the next read; the cached entry is untouched — asserted in `get_for_maestro_blanks_excluded_at_serve_time`). (e) **Migration high-water on base is `0016`** (403's `0015_maestro_state.sql` + 410's `0016_chat_messages_metadata.sql` already landed) — 413 adds none; `concerto_chat_full_chat_access` is a `workspaces.settings_json` JSON bool key, no column/migration/proto/CHECK-widen.
- **`concerto_chat_full_chat_access` mechanics:** new `workspaces.settings_json` JSON **bool** key, default `false`. Written via the net-new non-clobbering RMW `workspaces::set_settings_json_key` (verbatim mirror of `workareas::set_settings_json_key`: SELECT blob → parse object (`{}` on absent/malformed) → set key → re-serialize → UPDATE), read via `workspaces::get_settings_json_bool` (`None` ⇒ row/key absent OR non-bool value ⇒ caller defaults `false`). `PrivacyPolicy::summary_source(full_chat_access)` maps `true ⇒ FullLast3Turns`, `false/absent ⇒ SummaryOnly`. Round-trip + non-clobber-of-`permission_mode` + absent-key + non-bool-⇒-`None` are asserted in persist tests.
- **D10 `enterprise_data_privacy=false` debt:** Not touched here — `handlers/vcs.rs`'s hardcoded `false` in `FetchIssueByUrl` is **Task 411's** fix (coordinated, not duplicated). 413 enforces the resolver's `enterprise_data_privacy()` **as-is** (logic unchanged) in the Maestro summary/digest path only, via the `bool` the caller passes to `llm_gate`/`maestro_disabled_by_policy`. The `is_external_model` half of the gate is supplied by 412.
- **What 409/412/414/415 consume:** **405** (`get_workarea_summary`) + **409** (digest) call `SummaryCache::get_for_maestro`/`list_for_maestro` on the serve path (read-time-blanked `WorkareaSummary`, FROZEN by 404 §4.4); **412** consumes `PrivacyPolicy::{maestro_disabled_by_policy,llm_gate}` + `MaestroModelLocality` to gate its external provider and supplies the `is_external_model`/locality input from the live `MaestroProvider`, and consults `summarize_turn_gated`/`MAESTRO_DISABLED_BY_POLICY_SUMMARY` at the external call site; **414** publishes the `maestro.disabled_by_policy` event from the `MaestroLlmGate::DisabledExternalPolicy` decision; **415** renders the disabled banner. FROZEN surfaces: `PrivacyPolicy::{blank_excluded,summary_source,maestro_disabled_by_policy,llm_gate}`, `PRIVATE_WORKAREA_BLANK`, `MaestroLlmGate`, `MaestroModelLocality`, `SummarySource`, `summary::{get_for_maestro,list_for_maestro,summarize_turn_gated,refresh_summary_source,MAESTRO_DISABLED_BY_POLICY_SUMMARY}`, and `workspaces::{set_settings_json_key,get_settings_json_bool}` + the `concerto_chat_full_chat_access` key.
- **Deliberate debt:** NONE — the policy is fully implemented; `is_external_model` is a live typed input (`bool` / `MaestroModelLocality`), not a `todo!()` or hardwired constant.
- **Smoke-gate state:** **Unchanged** (no `scripts/smoke.d/*` / `scripts/smoke.manifest` change; the privacy policy is CI-provable in-process; real external-LLM egress remains the Phase-4 Tier-3 gate line "confirm an excluded workarea leaks only hard facts"). `cargo deny check` stayed green (no new crates; `serde_json` already a dep).
