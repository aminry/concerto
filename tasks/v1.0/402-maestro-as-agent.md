...
- **Drift from plan** — _(fill at impl time: e.g. whether the no-op parser pack is a new `MaestroPack` or reused `EchoPack`; exact `MaestroLaunchSpec` field names if they diverged from the frozen sketch; whether the scratch-cwd helper landed in `maestro/mod.rs` vs `provider.rs`; author-check result — confirm highest migration on `main` was still 0014 and the 0015/0016 reservation is intact.)_
- **Open questions for next task** — _(fill at impl time: **Task 412** extends `MaestroProvider` (Codex/Gemini LIVE + `DirectApiProvider` frozen-unwired + the daily budget + token counting) — note the exact trait method + `MaestroLaunchSpec` shape it builds on; **Task 406** consumes the strict ⇒ `MustAsk` matrix for write-tool confirmation chips; **Task 414** calls the spawn-config constructor at boot + adds the `enterpriseDataPrivacy`-disabled gate. Name the FROZEN surfaces each builds on.)_
- **Deliberate debt** — _(fill at impl time: confirm no `todo!()`/`unimplemented!()`; the 412-owned provider arms are left as a documented seam, NOT empty stubs; token accounting is intentionally absent — `ContextUsage{pct}` left untouched; boot-time lifecycle spawn intentionally deferred to 414.)_
- **Smoke-gate state** — _(fill at impl time: should remain **unchanged** — this task wires no boot-time Maestro spawn, so no `scripts/smoke.d/*`/`smoke.manifest` change; the maestro-digest capability is turned on by 414. Confirm `scripts/smoke.sh` not required for an `unchanged` task; note whether run.)_
