...
- **Drift from plan** —
  - **Provider trait extension:** added `CodexCliProvider`/`GeminiCliProvider` impls of 402's frozen `MaestroProvider` (trait + `ClaudeCliProvider` untouched). Refactored `ClaudeCliProvider`'s body into a private `resolve_cli_launch_spec(ctx, bin)` + `resolve_model(ctx)` helper so all three CLIs share the spawn shape and differ ONLY in the resolved binary (`{codex,gemini}_executable_path()` else bare `codex`/`gemini`). The frozen trait + launch-spec shape were NOT re-locked.
  - **Binary resolution site:** routed through the **provider** (each provider resolves its own bin from `ManagedPolicy`), NOT through `resolve_agent_bin`'s Maestro arm — `actor.rs` is outside this task's write-set and its Maestro arm still constructs `ClaudeCliProvider`. Threading the *selected* backend into that arm is 414's boot wiring (414 picks the provider via `select_provider` and feeds it the same way the arm currently feeds `ClaudeCliProvider`). The workarea `AgentKind::Codex|Gemini` arm at `actor.rs:1907` (the "deferred to Phase 3" error) is the **workarea** agent path, unrelated to the Maestro and left untouched.
  - **`parse_token_usage` source:** a per-backend `<int> input ... <int> output` end-of-turn usage-line scraper (case-insensitive, strips thousands-separators/trailing punctuation). Codex/Gemini share the CLI parser; `Direct` returns `None` (frozen seam, no stream). Returns `None` (logged at `debug`) on an unparseable turn — never panics, never under-counts with a silent 0. **Real CLI usage-line accuracy is the Tier-3 gate** (the recorded samples in tests are representative shapes, not a live round-trip).
  - **Budget wiring into 403:** `TokenBudget::record_usage` → `maestro_state::bump_daily_counters` (additive SQL); `reset` → `maestro_state::reset_budget`; boot hydration via `TokenBudget::from_state(&MaestroState)` over `maestro_state::get`. `maestro_state` is the single source of truth — the in-memory `TokenBudget` is a cache, so a Core restart mid-day resumes the cumulative count. 403's accessor signatures matched exactly (take `&mut SqliteConnection`); no surprises. Counters are `u64` in-memory / `i64` in the row (clamped on hydrate).
  - **UTC-midnight clock:** computed with pure integer math (`div_euclid`/`rem_euclid` on `86_400_000` ms) — **no `chrono` dependency added** to `concerto-core` (it has none; mirrors `log_filter`/`audit` day math). `next_utc_midnight_ms(now)` returns the first day-boundary strictly after `now`. `now_unix_ms` is injected (testable without a wall clock); 414 supplies the real instant.
  - **`OneShotLlm`/summarizer tokens:** kept **OUT** of this budget (the documented default, D5) — this budget counts only the interactive Maestro agent's tokens. The summarizer/digest tokens are 404/409's accounting via `OneShotLlm`.
  - **`disabled_by_policy` / on-prem Direct:** `select_provider` treats ALL `Direct` as external (`MaestroBackend::is_external_api()`), so `enterpriseDataPrivacy=true` + Direct ⇒ `ProviderSelection::DisabledByPolicy` unconditionally. On-prem Direct base-URL distinction (the "external Direct under privacy not selectable UNLESS on-prem" nuance) is deferred to the Direct-API fast-follow, which owns the base-URL config — noted as Tier-3/fast-follow debt below.
  - **Regen:** `./scripts/regen-interfaces.sh` produces **no diff** — its D4 contract reads only `crates/<crate>/src/api.rs`, and `concerto-core` has no `api.rs`, so core provider/budget types are not surfaced in `rust-api.md` (same as 402, which added `ClaudeCliProvider`/`MaestroProvider` with no rust-api entry). No `docs/interfaces/` change to commit.
  - **Windows cfg-gating fix (post-CI):** `crates/core/src/llm/provider.rs` imports `crate::maestro::provider::MaestroBackend`, but `maestro` is `#[cfg(unix)]` (over the agent supervisor), so the `build / windows-latest` lane failed `E0433: cannot find maestro in crate`. The whole module is Maestro-budget code and nothing outside the Maestro subsystem consumes it, so gated `pub mod provider;` **and** its `pub use provider::{…}` re-export in `llm/mod.rs` with `#[cfg(unix)]` (mirroring how `maestro` itself is gated). No logic/signature/behavior change; unix `cargo check`/`clippy -D warnings`/`fmt`/`concerto-core` tests stay green and the non-unix path no longer references `crate::maestro`.
- **Open questions for next task** —
  - **Task 414** reads (in `crate::llm::provider`): `MaestroLlmState::{is_inert, stale, set_inert, clear_inert, set_last_good_digest, digest_for_serving, backend}` + the typed `InertReason::{BudgetExhausted, DisabledByPolicy}` to publish `maestro.budget_exhausted` / `maestro.disabled_by_policy` on `maestro.events` (Event.checks_opaque=17); and drives the budget lifecycle: `TokenBudget::{from_state, record_usage, is_exhausted, reset, is_reset_due}` + `parse_token_usage(backend, raw)`. 414 also owns selecting the backend via `crate::maestro::provider::select_provider(...)` and feeding the chosen provider into `resolve_agent_bin`'s Maestro arm (or the Maestro launch site).
  - **Task 415** reads: `MaestroLlmState::{is_inert, stale, backend}` + `TokenBudget::{daily_in_today, daily_out_today, in_cap, out_cap}` for the banner + the 80%-amber/100%-red thresholds (R-10 default caps are `DEFAULT_DAILY_IN_CAP`/`DEFAULT_DAILY_OUT_CAP`; the user-configurable threshold *values* are 415's).
  - **Direct-API fast-follow** fills `DirectApiProvider::resolve_launch` (and the future native loop) behind the unchanged frozen seam; the constructor already exposes `provider()` (the `keychain::SecretKind::ProviderToken` lookup key) + `model()`.
- **Deliberate debt** —
  - `DirectApiProvider` is a signature-frozen seam: every method returns the typed `maestro.direct_api.unimplemented` marker (`DIRECT_API_UNIMPLEMENTED_MARKER`, helpers `direct_api_unimplemented()`/`is_direct_api_unimplemented()`) — **no macro, no empty-success**. The native function-call loop (Anthropic/OpenAI/Bedrock/Vertex/Foundry/OpenRouter request+tool-call+stream) + on-prem base-URL handling is the fast-follow (body-only change against the frozen seam; its HTTP-client pin is the fast-follow's, not this task's — `cargo deny` added no new crate).
  - **Tier-3 accuracy caveat:** `parse_token_usage` is validated only against recorded-shape samples; real Codex/Gemini end-of-turn usage-line accuracy (and which CLIs emit a parseable line at all) is the Phase-4 Tier-3 gate "confirm budget-exhaust goes inert while routing still works." Any CLI whose line does not match the `<int> input / <int> output` shape will return `None` (logged, turn unaccounted) until its arm is tuned.
- **Smoke-gate state** — `unchanged`; not re-run in-worktree (CI/operator gate for an `unchanged` task). The Maestro spine boots through the extended provider set with identical behavior absent an exhausted budget; `cargo check --workspace`, `clippy -D warnings`, `fmt --check`, `cargo deny`, and `cargo test --workspace --no-fail-fast` all green.
