...
## Definition of Done
- [ ] `Maestro.GetState` binding + `"Maestro.GetState"` in the `RpcMethod` union; `<BudgetBanner>` + `<DigestPanel>` fed real `MaestroState` (amber/red meter + blanked rows light up)
- [ ] Confirmation-chip producer subscribes to `GetState.maestro_session_id`'s `session.events`, lifts write-tool `AwaitingApproval` into `pendingConfirmation`, resolves via `Sessions.ResolveApproval` (no bypass, R-2)
- [ ] Per-workarea Maestro-visibility toggle calls `setWorkareaVisibility(FULL | HARD_FACTS_ONLY)` and reflects state
- [ ] Tier-2 tests pass (`pnpm -C apps/desktop typecheck|lint|test|build`); smoke unchanged
- [ ] No TODO/FIXME/unimplemented in new code; no `src-tauri`/proto/Rust touched; only `apps/desktop/**`
