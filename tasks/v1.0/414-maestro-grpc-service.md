...
- **Drift from plan** — <e.g. whether 401.5's skeleton already declared the `with_maestro_events` field/setter (lit up vs added); the exact `Status` codes chosen for policy-disabled vs budget-exhausted; whether the boot gate read `WorkspaceSettingsResolver` or `ManagedPolicy` (or both) and any default-model parsing surprise; any `maestro/mod.rs` region adjustment to avoid a rebase collision with sibling maestro tasks>
- **Open questions for next task** — <none expected downstream in Phase 4 — 414 is the W-cluster tail; note for **415** (the consumer) the exact `{"kind": ...}` `MaestroEvent` frame envelope it parses off `maestro.events`/`checks_opaque`, and confirm the `Digest` proto fields it renders are populated; flag for **412** the `MaestroEvent::BudgetExhausted` emission point this task left dormant until the live token counter trips it>
- **Deliberate debt** — <e.g. `BudgetExhausted` is emitted only when the handle reports exhaustion, which never happens until 412 wires counting — exercised by the test double only this task; the `notify_user` side-channel stays a 407 stub until 507 (P5) makes it live>
- **Smoke-gate state** — <`unchanged` (not re-run / re-run): this task adds no smoke capability; the boot path exits 0 with the handle present or absent; the `maestro-digest` smoke check is a later/Tier-3 concern>
