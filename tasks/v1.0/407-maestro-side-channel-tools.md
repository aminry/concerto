...
- **Drift from plan:** — (e.g. exact `notify_user`/`propose_chip` MCP input/output JSON 401 actually froze vs. the shapes assumed here; whether the registry hook took the backing handles by `Arc` field on the server state or by a closure; whether `MaestroChip.action` ended up an enum mirror vs. the free-form string; any rename of `NotifyRecorder`→`NotifySink` default impl)
- **Open questions for next task:** — **Task 507** (Phase 5) consumes the FROZEN `NotifyIntent`/`NotifySeverity` + the `NotifySink` seam to dispatch `notify_user` live through the P5 `NotificationHandle` (swap the recorder; tool body + MCP schema unchanged). **Task 409** (digest) appends its grouped next-step chips to **this** `ChipSlate` via `propose` (D11) and reuses `MaestroChip`. **Task 414** reads `ChipSlate::current()` to surface chips on the `Maestro` gRPC / `maestro.events` wire (proto FROZEN by 401.5). **Task 402** classifies `propose_chip` under the strict-mode `ToolClass` matrix (D4) — 407 only produces the chip. (Note for 507/414: the slate has no TTL by design — D11.)
- **Deliberate debt:** — `notify_user` is a **typed stub against 14**: it records the intent + returns `Ok` (so the Maestro believes the notification was accepted and does not retry), but performs **no real delivery** until Task 507 wires the live `NotificationHandle`. This is the README "`notify_user` (P4) stubs against 14 and is wired live in P5" precedent — it is **not** a `todo!()`/`unimplemented!()` macro and not an empty-silent-failure; it is a complete, tested, signature-frozen seam (`NotifySink`). No other debt.
- **Smoke-gate state:** — **Unchanged** (expected). 407 turns on no smoke capability; the two side-channel tools are CI-provable in-process (direct tool invocation + `NotifyRecorder::snapshot()` / `ChipSlate::current()` assertions); no `scripts/smoke.d/*` or `scripts/smoke.manifest` change. No new crates ⇒ `cargo deny` green; no proto/SQL/`api.rs` ⇒ `regen-interfaces.sh` is a no-op.
