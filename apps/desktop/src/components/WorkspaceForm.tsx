// Shared workspace form body — the assembly UI used by BOTH the create modal
// (`NewWorkspaceModal`) and the edit modal (a later task).
//
...
//     `cone_defaults`).
//
// The form owns all field/selection state and emits a `WorkspaceFormSubmit`
// (`repos: { repositoryId, sparseCones }[]`, sparseCones = [] for full) via
// `onSubmit`. The parent owns the create/update mutation and its lifecycle.
//
// CREATE mode additionally auto-fills the Name from the selected repo names
// (design Part 1) until the user edits the field manually.
...
/// Initial values to seed the form (edit mode pre-fill, or create defaults).
...
/// The assembled values handed to the parent on submit.
...
export type WorkspaceFormProps = {
  mode: "create" | "edit"; // "create" enables auto-name
  initial?: WorkspaceFormInitial;
  submitLabel: string;
  pendingLabel: string;
  pending: boolean;
  externalError?: string | null;
  notice?: string | null; // shown above the buttons (edit mode w/ workareas)
  onCancel: () => void;
...
export function WorkspaceForm({
  ...
  // Whether the user has manually edited the Name field. In edit mode the
  // name is "owned" from the start (no auto-fill); in create mode it's only
  // owned once the user types a non-empty value.
  ...
  // Part-1 auto-name (CREATE mode only): until the user edits the Name field,
  // derive it from the selected repo names (in selection order).
  ...
  useEffect(() => {
    if (mode === "create" && !nameEdited) {
      setName(deriveWorkspaceName(selectedNames));
    }
  ...
  const canSubmit =
    name.trim().length > 0 && selectionOrder.length > 0 && !pending;
  ...
  function handleSubmit(e: React.FormEvent): void {
    ...
    onSubmit({
      ...
      repos: selectionOrder.map((id) => ({
        repositoryId: id,
        sparseCones:
          selected[id]?.mode === "sparse" ? selected[id].cones : [],
      })),
    });
  }
  ...
  return (
    <form className="space-y-4" onSubmit={handleSubmit}>
      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-2">
          <Button
            ...
            size="sm"
            onClick={() => setAddSource((s) => (s === "url" ? null : "url"))}
          >
        ...
        {addSource === "url" && (
          <AddByUrlPanel onAdded={afterRepoAdded} onError={setErrorMsg} />
        )}
        {addSource === "local" && (
      ...
      {(externalError ?? errorMsg) && (
        <p className="text-xs text-err">{externalError ?? errorMsg}</p>
      )}
