...
impl CoreUnderTest {
    ...
    ///
    /// Same connection semantics as [`Self::runtime_client`]: each call
    /// dials a fresh channel.
    pub async fn repositories_client(&self) -> Result<RepositoriesClient> {
        Ok(clients::repositories_client(self.socket_path.clone()).await?)
    }

    /// Connect a Tonic [`WorkspacesClient`] to the running Core (Task 19).
    ///
    ...
    /// Connect a Tonic [`SessionsClient`] to the running Core (Task 23).
    ///
    /// Same connection semantics as [`Self::runtime_client`].
    pub async fn sessions_client(&self) -> Result<SessionsClient> {
        Ok(clients::sessions_client(self.socket_path.clone()).await?)
    }

    /// Connect a Tonic [`StreamsClient`] to the running Core (Task 23).
    ///
    /// Same connection semantics as [`Self::runtime_client`].
